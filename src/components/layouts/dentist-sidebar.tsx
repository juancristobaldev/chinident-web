...
const dentistLinks = [
  ...
  { href: "/dentist/profile", label: "Perfil", icon: UserCircle },
];
  ...
  return (
    <div className="flex h-full flex-col bg-background">
      <div className={cn("flex h-16 items-center border-b px-4", isCollapsed ? "justify-center" : "justify-between")}>
        {!isCollapsed && (
          <Link href="/dentist/dashboard" className="flex items-center gap-2 font-bold text-xl tracking-tight hover:opacity-80 transition-opacity" title="ChiniDent">
            <div className="bg-emerald-500/10 p-1.5 rounded-lg shrink-0">
              <ToothIcon className="h-5 w-5 text-emerald-600" />
            </div>
            <span className="truncate">ChiniDent</span>
          </Link>
        )}
        {onToggle && (
          <Button 
        ...
        <nav className="space-y-1 px-3">
          ...
          {dentistLinks.map((link) => {
            ...
            return (
              ...
              >
                {isActive && (
                  <div className={cn("absolute top-1/2 -translate-y-1/2 h-full w-1 bg-emerald-600", isCollapsed ? "left-0 rounded-r-md" : "left-0 rounded-r-md")} />
                )}
                <link.icon className={cn("h-5 w-5 shrink-0 transition-colors", isActive ? "text-emerald-600" : "text-muted-foreground group-hover:text-accent-foreground")} />
